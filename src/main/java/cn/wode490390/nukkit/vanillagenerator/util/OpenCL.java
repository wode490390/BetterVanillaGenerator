package cn.wode490390.nukkit.vanillagenerator.util;

import cn.wode490390.nukkit.vanillagenerator.BetterGenerator;
...
public class OpenCL {
    ...
    private static final ClassLoader CLASS_LOADER = OpenCL.class.getClassLoader();
            ...
            if (openCLDir.exists() && openCLDir.isDirectory()) {
                ...
                if (file.exists()) {
                    try (InputStream input = new FileInputStream(file)) {
                        ...
                        BetterGenerator.getInstance().getLogger().warning("Could not load custom OpenCL program.", ex);
                    }
                } else {
                    try (InputStream input = CLASS_LOADER.getResourceAsStream(name)) {
                        CLProgram program = context.createProgram(input).build();
                        programs.put(name, program);
                        return program;
                    } catch (IOException ex) {
                        BetterGenerator.getInstance().getLogger().warning("Could not load builtin OpenCL program.", ex);
                    }
                }
            }
        }
    ...
    public static void initContext(CLPlatform platform) {
        openCLDir = new File(BetterGenerator.getInstance().getDataFolder(), "opencl");

        if (!openCLDir.isDirectory() && !openCLDir.mkdirs()) {
            BetterGenerator.getInstance().getLogger().error("Cannot create OpenCL directory");
        }
        ...
        BetterGenerator.getInstance().getLogger().info("OpenCL: Using " + platform + " on device " + device + ".");
    }
